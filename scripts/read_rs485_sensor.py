...
def read_registers(port: serial.Serial, addr: int, register: int, count: int) -> list[int]:
...
def read_sen0482(port: serial.Serial, addr: int) -> None:
    """SEN0482 Wind Direction."""
    regs = read_registers(port, addr, 0x0000, count=2)
    degrees = regs[0] / 10.0
...
def read_sen0483(port: serial.Serial, addr: int) -> None:
    """SEN0483 Wind Speed."""
    regs = read_registers(port, addr, 0x0000, count=1)
...
def read_sen0644(port: serial.Serial, addr: int) -> None:
    """SEN0644 Illuminance."""
    regs = read_registers(port, addr, 0x0002, count=2)
    ...
    with serial.Serial(args.port, baudrate=_BAUD, timeout=_TIMEOUT_S) as port:
        if args.loop:
            ...
            try:
                ...
                print(f"  ERROR: {exc}")
                print("  Check wiring, power, address, and that pyserial is installed.")
                sys.exit(1)
